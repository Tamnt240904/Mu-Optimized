...
class AttributionResult:
    ...
    optimizer_info: dict = field(default_factory=dict)
...
def compute_weight_diagnostics(
    ...
    mu = mu.detach()
    return {
        "mu_l2_sq": float((mu ** 2).sum()),
        "mu_entropy": float(-(mu * torch.log(mu + eps)).sum()),
        "mu_max": float(mu.max()),
        "mu_min": float(mu.min()),
        "mu_active_count": int((mu > active_threshold).sum().item()),
        "mu_sum": float(mu.sum()),
    }
...
def compute_residual_diagnostics(
    ...
    residual = d.detach() - delta_f.detach()
    mu = mu.detach()
    return {
        "weighted_residual_l1": float((mu * residual.abs()).sum()),
        "weighted_residual_l2": float(torch.sqrt((mu * residual ** 2).sum())),
    }
