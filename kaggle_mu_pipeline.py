#!/usr/bin/env python3
"""Kaggle/CLI pipeline for Mu-Optimized ImageNet evaluation.

This script reproduces the workflow from the Kaggle notebook:
...
6. Optionally archive results/logs for Kaggle download.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Mu-Optimized Kaggle pipeline from CLI.")
    ...
    parser.add_argument("--results-dir", type=Path, default=Path("/kaggle/working/results"))
    parser.add_argument("--logs-dir", type=Path, default=Path("/kaggle/working/logs"))
...
def maybe_run(cmd: list[str], output: Path | None, log: Path | None, cwd: Path, skip_existing: bool) -> None:
    if skip_existing and output is not None and output_exists(output):
        print(f"SKIP existing output: {output}")
        return
    run_streaming(cmd, log_path=log, cwd=cwd)
...
def main() -> None:
    ...
    if args.run_small_test:
        ...
        maybe_run(
            prepare_test_cmd,
            output=test_root / "selected.csv",
            log=logs_dir / f"test_prepare_first{args.test_first_count}.log",
            cwd=repo_dir,
        ...
        for model in args.models:
            ...
            maybe_run(cmd, output=out, log=log, cwd=repo_dir, skip_existing=args.skip_existing)
    ...
    if not args.skip_prepare:
        ...
        maybe_run(
            prepare_cmd,
            output=selected_csv,
            log=logs_dir / f"full_prepare_first{args.first_count}.log",
            cwd=repo_dir,
            skip_existing=args.skip_existing,
        )
    else:
    ...
    if not args.skip_flatten:
        (results_dir / "flat").mkdir(parents=True, exist_ok=True)
        for model in args.models:
            in_json = full_json_paths[model]
            out_csv = results_dir / "flat" / f"flat_{model}_first{args.first_count}_correct_N{args.steps}_tau{args.tau}_{args.auc_score}.csv"
            cmd = [
                sys.executable,
                "-u",
                "flatten_insdel_json.py",
            ...
            maybe_run(cmd, output=out_csv, log=logs_dir / f"flatten_{model}.log", cwd=repo_dir, skip_existing=args.skip_existing)
            flat_paths[model] = out_csv
    else:
        print("skip_flatten=True; using existing flat CSV files.")
        for model in args.models:
            flat_paths[model] = results_dir / "flat" / f"flat_{model}_first{args.first_count}_correct_N{args.steps}_tau{args.tau}_{args.auc_score}.csv"
    ...
    if not args.skip_selection:
        if "resnet50" not in flat_paths or "vgg16" not in flat_paths:
            ...
            selection_dir = results_dir / f"selection_first{args.first_count}_N{args.steps}_tau{args.tau}_{args.auc_score}"
            cmd = [
                sys.executable,
                "-u",
                "select_mu_better_two_models.py",
            ...
            maybe_run(cmd, output=selection_dir / "merged_per_image_metrics.csv", log=logs_dir / "select_mu_better_two_models.log", cwd=repo_dir, skip_existing=args.skip_existing)
    ...
    # 6. Archive.
    if args.archive:
        archive = Path("/kaggle/working") / f"mu_pipeline_results_first{args.first_count}_N{args.steps}_tau{args.tau}_{args.auc_score}.zip"
