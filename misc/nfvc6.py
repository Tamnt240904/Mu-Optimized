...
def compute_guided_ig(model_f, x, x_prime, target_class, N=100, fraction=0.25):
    """
    Guided IG: at each step, advance the dimensions with the SMALLEST
    |∇f| first. Leave the high-gradient (important) dimensions for last.

    Logic: clear the "boring" background dimensions early so that when
    the logit finally jumps (late in the path), it's because of specific,
    identifiable high-gradient features — not a mix of signal and noise.

    Result: logit profile is FLAT early (boring dims don't affect output),
    then JUMPS late (important dims finally change → model recognizes object).
    ...
    for i in range(N):
        ...
        # Score: |∇f| * |remaining|
        # We want to advance LOWEST-score dimensions first
        score = (grad_f.abs() * remaining.abs()).flatten(1)  # (B, D)

        # Select BOTTOM-fraction dimensions (smallest gradient × remaining)
        k = max(1, int(fraction * score.shape[1]))
        # topk on NEGATIVE score → bottom-k
        _, bottom_idx = (-score).topk(k, dim=1)

        step = torch.zeros_like(z).flatten(1)
        remaining_flat = remaining.flatten(1)

        for b in range(B):
            step[b, bottom_idx[b]] = remaining_flat[b, bottom_idx[b]] * dt / (1.0 / N)
...
def compute_idgi(model_f, x, x_prime, target_class, N=100, chunk=16):
    """
    IDGI: Importance-Driven Gradients Integration.

    From: "IDGI: A Framework to Eliminate Explanation Noise from
    Integrated Gradients" (2023)

    Key idea: at each interpolation point, normalize the gradient to
    a unit direction vector, then weight by the local logit change.

    For each step i:
        direction_i = ∇f(zᵢ) / ‖∇f(zᵢ)‖    (unit gradient direction)
        weight_i = |f(zᵢ) - f(zᵢ₋₁)|         (importance = logit change)

    Attribution per dimension j:
        A_j = Δf · Σᵢ [wᵢ · direction_i_j] / Σᵢ wᵢ

    This guarantees completeness: Σ_j A_j = Δf by construction,
    because the weights are normalized and scaled by Δf.

    The effect: at steps where the logit barely changes (flat regions),
    the weight is ~0 → those noisy gradients are suppressed.
    At steps where the logit jumps (feature recognition), weight is high
    → those gradients dominate the attribution.
    """
    device = x.device
    B = x.shape[0]
    direction = x - x_prime
    ...
    delta_f = (logit_vals[-1] - logit_vals[0])  # (B,)

    # Importance weights: |Δlogit| at each step
    weights = []
    for i in range(1, N + 1):
        w = (logit_vals[i] - logit_vals[i - 1]).abs()
        weights.append(w)
    weights_stack = torch.stack(weights, 0)  # (N, B)
    w_sum = weights_stack.sum(dim=0, keepdim=True).clamp(min=1e-8)  # (1, B)
    norm_weights = weights_stack / w_sum  # (N, B), sums to 1
    ...
    # Normalize each gradient to unit direction
    grads_flat = grads.reshape(N, B, -1)  # (N, B, D)
    grad_norms = grads_flat.norm(dim=2, keepdim=True).clamp(min=1e-10)  # (N, B, 1)
    unit_grads = grads_flat / grad_norms  # (N, B, D)

    # Weighted sum of unit directions: Σ wᵢ · direction_i
    # norm_weights: (N, B) → (N, B, 1)
    w_exp = norm_weights.unsqueeze(-1)  # (N, B, 1)
    weighted_dirs = (w_exp * unit_grads).sum(dim=0)  # (B, D)

    # Scale by Δf to guarantee completeness
    # A_j = Δf · weighted_dir_j  →  Σ A_j = Δf · Σ weighted_dir_j
    # We need to normalize so Σ_j weighted_dir_j = 1
    dir_sum = weighted_dirs.sum(dim=1, keepdim=True).clamp(min=1e-10)  # (B, 1)
    attr_flat = delta_f.unsqueeze(-1) * weighted_dirs / dir_sum  # (B, D)
    attr = attr_flat.view_as(x)

    # Per-step density for profiling
    densities = []
    for i in range(N):
        d_flat = delta_f.unsqueeze(-1) * w_exp[i] * unit_grads[i] / dir_sum
        densities.append(d_flat.sum(dim=1).item())

    # Path (same straight line as IG)
...
def demo():
    ...
    # ==========================================
    # 4. NEURAL FLOW IG (straight-line → HVP refinement)
    # ==========================================
    print(f"\n{'=' * 50}")
    print("4. Neural Flow IG (IG straight-line → HVP refinement)")
    print(f"{'=' * 50}")
    t0 = time.perf_counter()
    ...
    # Initialize from straight-line path (not Guided IG)
    # Guided IG already has low gradients early → HVP has nothing to minimize.
    # Straight line has high gradients everywhere → HVP can meaningfully
    # redirect waypoints to lower-energy regions.
    nf_path, refine_history = refine_path_hvp(
        model, ig_path, x, x_prime, pc, cfg=cfg, log_every=5)
    ...
    print(
        f"  Compl. error      {abs(ig_sum - delta_f):10.3f} {abs(idgi_sum - delta_f):10.3f} {abs(gig_sum - delta_f):12.3f} {abs(nf_sum - delta_f):10.3f}")
    ...
    print(
        f"  Energy reduction  {'---':>10} {energy_reduction_idgi:+9.1f}% {energy_reduction_gig:+11.1f}% {energy_reduction_nf:+9.1f}%")
    print(f"{'=' * 75}")
