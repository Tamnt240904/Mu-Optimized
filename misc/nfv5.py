...
def plot_comparison(x, attr_ig, attr_gig, attr_nf, info, save_path=None):
    ...
    def heatmap(a):
        return a[0].cpu().abs().sum(0).numpy()

    h_ig = heatmap(attr_ig)
    h_gig = heatmap(attr_gig)
    h_nf = heatmap(attr_nf)
    vmax = max(h_ig.max(), h_gig.max(), h_nf.max())

    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
    axes[0].imshow(img); axes[0].set_title("Input"); axes[0].axis("off")
    axes[1].imshow(h_ig, cmap="hot", vmax=vmax); axes[1].set_title("Vanilla IG"); axes[1].axis("off")
    axes[2].imshow(h_gig, cmap="hot", vmax=vmax); axes[2].set_title("Guided IG"); axes[2].axis("off")
    axes[3].imshow(h_nf, cmap="hot", vmax=vmax); axes[3].set_title("Neural Flow IG"); axes[3].axis("off")

    fig.suptitle(f"Completeness — IG:{info['ig_compl']:.2f}  "
...
def demo():
            ...
            try:
                ...
                for i in range(min(500, len(ds))):
                    ...
                    if c.item() >= MIN_CONF:
                        ...
                        print(f"ImageNet idx={i}, class={pc}, conf={cf:.4f}")
                        loaded = True; break
            except Exception:
                pass
            if loaded: break
    ...
    # ==========================================
    print(f"\n{'='*50}")
    print("2. Guided Integrated Gradients")
    print(f"{'='*50}")
    t0 = time.perf_counter()
    ...
    # ==========================================
    print(f"\n{'='*50}")
    print("3. Neural Flow IG (Guided IG → HVP refinement)")
    print(f"{'='*50}")
    t0 = time.perf_counter()
    ...
    # ==========================================
    print(f"\n{'='*60}")
    print(f"{'':20} {'IG':>10} {'Guided IG':>12} {'NF-IG':>10}")
    print(f"{'-'*60}")
    print(f"  Time (s)          {t_ig:10.1f} {t_gig:12.1f} {t_refine:10.1f}")
    print(f"  Σ attr            {ig_sum:+10.3f} {gig_sum:+12.3f} {nf_sum:+10.3f}")
    print(f"  Compl. error      {abs(ig_sum-delta_f):10.3f} {abs(gig_sum-delta_f):12.3f} {abs(nf_sum-delta_f):10.3f}")
    print(f"  Path energy       {ig_energy:10.0f} {gig_energy:12.0f} {nf_energy:10.0f}")
    ...
    print(f"  Energy reduction  {'---':>10} {energy_reduction_gig:+11.1f}% {energy_reduction_nf:+9.1f}%")
    print(f"{'='*60}")
    ...
    try:
        plot_comparison(x, attr_ig, attr_gig, attr_nf, info, "attr_v5.png")
    ...
    return attr_ig, attr_gig, attr_nf, info
