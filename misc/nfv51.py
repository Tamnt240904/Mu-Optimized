...
def plot_comparison(x, attr_ig, attr_gig, attr_nf, info, save_path=None):
    ...
    def heatmap(a):
        ...
        # Per-image normalization: clip at 99th percentile for contrast
    ...
    # Each heatmap normalized independently to [0, 1] for visibility
    axes[1].imshow(h_ig, cmap="hot", vmin=0, vmax=1)
    axes[1].set_title("Vanilla IG");
    axes[1].axis("off")

    axes[2].imshow(h_gig, cmap="hot", vmin=0, vmax=1)
    axes[2].set_title("Guided IG");
    axes[2].axis("off")

    axes[3].imshow(h_nf, cmap="hot", vmin=0, vmax=1)
    axes[3].set_title("Neural Flow IG");
    axes[3].axis("off")
...
def demo():
    ...
    print(
        f"  Compl. error      {abs(ig_sum - delta_f):10.3f} {abs(gig_sum - delta_f):12.3f} {abs(nf_sum - delta_f):10.3f}")
