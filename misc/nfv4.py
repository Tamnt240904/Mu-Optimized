...
class LossW:
    ...
    linearity: float = 2.0       # each step's ∇f·v ≈ Δf (uniform density)
    vel_reg: float = 0.01
...
class Metrics:
    ...
    delta_f: float = 0.
    endpt_l2: float = 0.
    mean_grad_norm: float = 0.


def compute_loss(model_f, v_theta, x, x_prime, target_class, N=20,
                 w=LossW(), chunk=8, phase=2):
    """
    Phase 1: boundary + vel_reg
    Phase 2: + completeness + linearity

    LINEARITY: for ideal f(γ(α)) = f(x') + α·Δf, need ∇f·v = Δf
    at every step. Per-step density dᵢ = ∇f(γᵢ)·vᵢ should be constant.
    ∇f detached, v connected → clean gradient to v_theta. Not exploitable.
    """
    B = x.shape[0]
    device = x.device
    ...
    L_bnd = ((traj[-1] - x) ** 2).flatten(1).sum(1).sqrt().mean()
    vel_stack = torch.stack(vels, 0)
    L_vreg = (vel_stack ** 2).mean()
    endpt = L_bnd.item()

    if phase == 1:
        loss = w.boundary * L_bnd + w.vel_reg * L_vreg
        return loss, Metrics(total=loss.item(),
                             boundary=w.boundary * L_bnd.item(),
                             vel_reg=w.vel_reg * L_vreg.item(),
                             delta_f=delta_f.mean().item(),
                             endpt_l2=endpt)

    sel_traj = [traj[i + 1] for i in range(N)]
    sel_vels = [vels[i] for i in range(N)]
    mg = model_grads_batched(model_f, sel_traj, target_class, chunk=chunk)
    vs = torch.stack(sel_vels, 0)

    mg_f = mg.reshape(N, B, -1)
    vs_f = vs.reshape(N, B, -1)

    # Per-step attribution density
    dot = (mg_f * vs_f).sum(dim=2)  # (N, B), connected via vs_f

    # COMPLETENESS: mean density ≈ Δf
    path_int = dot.mean(dim=0)
    L_compl = ((path_int - delta_f) ** 2 / (delta_f ** 2 + 1.0)).mean()

    # LINEARITY: all steps should have EQUAL attribution density
    # Instead of matching each dᵢ to Δf (which fights boundary by
    # forcing speed changes), minimize the VARIANCE of dᵢ.
    # This says "distribute attribution uniformly" without constraining
    # the path speed. Completeness handles the total.
    dot_mean = dot.mean(dim=0, keepdim=True)  # (1, B) — mean density
    L_linear = ((dot - dot_mean) ** 2 / (dot_mean ** 2 + 1.0)).mean()

    mean_gn = mg_f.norm(dim=2).mean().item()

    loss = (w.boundary * L_bnd
            + w.completeness * L_compl
            + w.linearity * L_linear
            + w.vel_reg * L_vreg)
    ...
    return loss, Metrics(
        ...
        linearity=w.linearity * L_linear.item(),
        vel_reg=w.vel_reg * L_vreg.item(),
        ...
        for step in range(phase1_steps):
            ...
            if step % log_every == 0 or step == phase1_steps - 1:
                ...
                print(f"  [{step:3d}/{phase1_steps}] "
                      ...
                      f"vreg={m.vel_reg:.4f} endpt={m.endpt_l2:.4f}")
        ...
        for step in range(phase2_steps):
            ...
            if step % log_every == 0 or step == phase2_steps - 1:
                ...
                print(f"  [{step:3d}/{phase2_steps}] "
                      f"L={m.total:.4f} "
                      f"lin={m.linearity:.4f} "
                      f"compl={m.completeness:.4f} "
                      f"bnd={m.boundary:.4f} "
                      f"vreg={m.vel_reg:.4f} "
                      f"| ∫={m.path_int:.3f} Δf={m.delta_f:.3f} "
                      f"endpt={m.endpt_l2:.4f}")

            if patience >= 200 and step > 200:
...
def demo():
    ...
    attr_ode, info = exp.explain(
        x, x_prime, pc,
        phase1_steps=250, phase2_steps=800,
        N=20, N_eval=50, lr=3e-3,
        w=LossW(boundary=50, completeness=5, linearity=1, vel_reg=0.01),
        chunk=8, log_every=100,
    )
